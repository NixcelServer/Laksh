...
const CategoryPage = () => {
  ...
  const selectedCat = useSelector((state) => state.userReducer.selectedCategory);
  console.log("in home categories",selectedCat)
...
console.log("in assign  cat", encCatId);
  ...
  const getCategoryDetails = async () => {
    if (encCatId) {
      try {
        ...
        // setCategoryName(category.name); // Assuming 'name' is the property holding the category name
      } catch (error) {
...
setShowProductDetails(true);
  ...
  return (
    <div className="main-content" style={{ margin: '100px auto', maxWidth: '1200px', padding: '0 15px', backgroundColor: 'white' }}>
      ...
      <h1>{encCatId}</h1>
      {/* Display other category-related information here */}
    {/* </div>  */}
      ...
      <section className="section">
        <div className="section-body">
          {productsBySubCategory.subCategories.map((subCategory, index) => (
            <div key={index}>
              <div className="mb-2 text-left" style={{ color: 'black', fontWeight: 'bold', fontSize: '1.5rem' }}>
                {subCategory.subcategory.sub_cat_name}
              </div>
              <div className="row">
                {subCategory.products.slice(0, showAllProducts ? subCategory.products.length : 6).map((product, prodIndex) => (
                  <div className="col-12 col-md-3 col-lg-2" key={prodIndex}>
                    <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'nowrap', overflowX: 'auto' }}>
                      <div style={{ marginRight: '10px' }}>
                        <div className="mb-2 text-muted">{product.prod_name}</div>
                        <img
