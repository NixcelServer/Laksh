import { Box, Flex, Grid, Heading, Text, GridItem, useDisclosure, HStack, Link, Icon, Menu, MenuButton, MenuItem, MenuList, Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, ModalBody, ModalFooter, Button } from '@chakra-ui/react'
...
export default function HomeGrid({ title, single, info, onTitleClick, posts, cat }) {
  ...
  const generateImageUrl = (path) => {
    console.log( `${baseURL}storage/app/${path}`);
  ...
  return (
    <>
      <Box m='10px' p='30px' boxShadow='md' rounded='md' width="100%" overflow="hidden">
        ...
        <Flex flexDirection={{ base: "column", md: 'row' }}>
          ...
          <div
            ...
            >
              <img
                src={generateImageUrl(single.img)}
                alt={title}
                style={{
                  height: '100%',
                  width: '100%',
      ...
      <Box m='10px' p='30px' boxShadow='md' rounded='md' width='100%' height={{ base: 'auto', md: '500px' }} overflow='hidden'>
        <hr style={{ border: '1px solid teal', marginTop: '-4px' }} />
            ...
            {posts.map((post, index) => (
              <GridItem key={index} colSpan={1}>
                <ProductBox
                  ...
                  other_specifications={post.other_specifications}
