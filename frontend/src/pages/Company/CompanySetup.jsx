...
const checkGSTNo = (e) => {
  ...
  if (name === 'gst_no' && !gstRegex.test(value)) {
    // If it doesn't match the pattern, set an error message
    setGstNoError(" It should be 15 characters long.The first 2 characters should be a number.The next 10 characters should be the PAN number of the taxpayer.The 13th character (entity code) should be a number from 1-9 .The 14th character should be Z.The 15th character should be a number."
    );
  } else {
    // If it matches the pattern or for other fields, clear any previous error message
    setGstNoError('');
...
const checkIFSC = (e) => {
  ...
  //useEffect(() => {
    // Check if there's a file stored in localStorage when the component mounts
  //   const storedFile = localStorage.getItem('selectedFile');
  //   if (storedFile) {
  //     setFormData({ ...formData, file: JSON.parse(storedFile) });
  //   }
  // }, []);
  ...
  //   setFormData({
  //     ...formData,
  //     country: e.target.value // Update selectedCountry with the selected value
  //   });
  ...
  // //   axios.get(`https://api.countrystatecity.in/v1/countries/[ciso]/states${countryCode}`)
  // //     .then(response => {
                                  ...
                                  <div>
                                    ...
                                    <div style={{ marginBottom: "1rem" }}>
                                      <Flex justifyContent={["center", "flex-end"]} flexDirection={["column", "row"]}>
                                        ...
                                        name="c_annual_to" // Add name attribute
